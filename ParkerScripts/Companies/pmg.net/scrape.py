...
def scrape_job_postings(file_path):
    ...
    try:
        ...
        try:
            job_elements = driver.find_elements(By.CSS_SELECTOR, "div.fusion-content-boxes")
            for job_element in job_elements:
                try:
                    title_element = job_element.find_element(By.CSS_SELECTOR, "h2.content-box-heading")
                    title = title_element.text.strip() if title_element.text.strip() else title_element.get_attribute('innerHTML').strip()
                except NoSuchElementException:
                    title = "No Title"
                
                try:
                    url_element = job_element.find_element(By.CSS_SELECTOR, "a.heading-link")
