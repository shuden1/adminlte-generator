...
def scrape_job_listings(file_path):
    ...
    try:
        job_elements = driver.find_elements(By.CSS_SELECTOR, "div.wp-block-media-text.alignwide")
        for job_element in job_elements:
            try:
                title_element = job_element.find_element(By.CSS_SELECTOR, "h2.wp-block-heading")
                title = title_element.text.strip() or title_element.get_attribute('innerHTML').strip()
            except NoSuchElementException:
                title = "No Title"

            try:
                url_element = job_element.find_element(By.CSS_SELECTOR, "a.cta-blue")
