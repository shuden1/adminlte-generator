...
def scrape_jobs(file_path):
    ...
    try:
        ...
        for job_element in job_elements:
            try:
                title_element = job_element.find_element(By.CSS_SELECTOR, "h2.headline.headline__medium.headline--no-spacing")
                title = title_element.text.strip() if title_element.text.strip() else title_element.get_attribute('innerHTML').strip()

                try:
                    url_element = job_element.find_element(By.CSS_SELECTOR, "a[data-job-id]")
