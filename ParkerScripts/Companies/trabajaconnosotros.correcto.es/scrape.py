...
def scrape_jobs(file_path):
    ...
    try:
        ...
        job_openings = []
        job_blocks = driver.find_elements(By.CSS_SELECTOR, '.job, .jobs')

        for job_block in job_blocks:
            job_title_elements = job_block.find_elements(By.CSS_SELECTOR, 'a')
            for job_title_element in job_title_elements:
                job_title = job_title_element.get_attribute('innerText').strip()
