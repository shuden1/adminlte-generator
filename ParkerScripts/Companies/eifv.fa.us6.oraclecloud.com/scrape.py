...
def scrape_job_listings(file_path):
    ...
    try:
        ...
        job_elements = driver.find_elements(By.CSS_SELECTOR, "div.search-results.job-tile.job-list-item")

        for job_element in job_elements:
            try:
                title_element = job_element.find_element(By.CSS_SELECTOR, "span.job-tile__title")
                title = title_element.text.strip() if title_element.text.strip() else title_element.get_attribute('innerHTML').strip()
            except NoSuchElementException:
                title = "No Title Found"

            try:
                url_element = job_element.find_element(By.CSS_SELECTOR, "div.job-list-item__link")
