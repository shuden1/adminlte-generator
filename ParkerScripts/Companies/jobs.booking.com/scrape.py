...
def scrape_jobs(file_path):
    ...
    try:
        ...
        for job_element in job_elements:
            try:
                title_element = job_element.find_element(By.CSS_SELECTOR, "div.position-title.line-clamp.line-clamp-2")
                job_title = title_element.text.strip() if title_element.text.strip() else title_element.get_attribute('innerHTML').strip()
            except NoSuchElementException:
                job_title = "No Title"

            try:
                job_url = job_element.get_attribute('role') if job_element.get_attribute('role') == "link" else "#"
            except NoSuchElementException:
                job_url = "#"
