...
def scrape_jobs(file_path):
    ...
    try:
        job_elements = driver.find_elements(By.CSS_SELECTOR, "div.career-item")
        for job_element in job_elements:
            try:
                title_element = job_element.find_element(By.CSS_SELECTOR, "h1.card-careers__title")
                title = title_element.text.strip() if title_element.text.strip() else title_element.get_attribute('innerHTML').strip()
            except NoSuchElementException:
                title = "No Title"

            try:
                url_element = job_element.find_element(By.CSS_SELECTOR, "a.card-careers")
