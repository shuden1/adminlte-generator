...
def scrape_job_postings(file_path):
    ...
    try:
        job_elements = driver.find_elements(By.CSS_SELECTOR, "div.current-openings-item")
        for job_element in job_elements:
            try:
                title_element = job_element.find_element(By.CSS_SELECTOR, "span.current-opening-title")
                title = title_element.text.strip() if title_element.text.strip() else title_element.get_attribute('innerHTML').strip()
            except NoSuchElementException:
                title = "No Title"

            try:
                url_element = job_element.find_element(By.CSS_SELECTOR, "button.fa.fa-chevron-right.myapp-pi-margin.vdl-button.vdl-button--link")
