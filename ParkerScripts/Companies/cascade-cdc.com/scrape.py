...
def scrape_job_listings(file_path):
    ...
    try:
        job_elements = driver.find_elements(By.CSS_SELECTOR, 'li.jobInfo.JobListing[aria-label="job postings"]')
        for job_element in job_elements:
            try:
                title_element = job_element.find_element(By.CSS_SELECTOR, 'span.jobInfoLine.jobTitle[role="heading"]')
                title = title_element.text.strip() if title_element.text.strip() else title_element.get_attribute('innerHTML').strip()
            except NoSuchElementException:
                title = "No Title Found"

            try:
                url_element = job_element.find_element(By.CSS_SELECTOR, 'a.JobListing__container[href]')
