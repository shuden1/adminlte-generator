...
def scrape_jobs(file_path):
    ...
    try:
        ...
        try:
            job_openings = driver.find_elements(By.CSS_SELECTOR, "SPECIFY_JOB_OPENING_SELECTOR_HERE")
            for job in job_openings:
                try:
                    job_title_element = job.find_element(By.CSS_SELECTOR, "SPECIFY_JOB_TITLE_SELECTOR_HERE")
                    job_title = job_title_element.text.strip() or job_title_element.get_attribute('innerHTML').strip()
                except NoSuchElementException:
                    job_title = "No Title Found"

                try:
                    job_url_element = job.find_element(By.CSS_SELECTOR, "SPECIFY_JOB_URL_SELECTOR_HERE")
