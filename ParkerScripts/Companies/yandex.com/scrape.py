...
def scrape_job_listings(file_path):
    ...
    try:
        ...
        for job_element in job_elements:
            try:
                title_element = job_element.find_element(By.CSS_SELECTOR, "div.lc-jobs-text.lc-jobs-text_type_header.lc-jobs-text_size_s.lc-jobs-vacancy-card__header")
                title = title_element.text.strip() if title_element.text.strip() else title_element.get_attribute('innerHTML').strip()
            except NoSuchElementException:
                title = "No Title Found"

            try:
                url_element = job_element.find_element(By.CSS_SELECTOR, "a.lc-link.lc-jobs-link.lc-jobs-link_view_tag")
