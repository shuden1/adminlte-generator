...
def scrape_jobs(html_file):
    ...
    try:
        job_openings = driver.find_elements(By.CSS_SELECTOR, "div.ct-div-block.flipbox__front")
        for job in job_openings:
            try:
                job_title_element = job.find_element(By.CSS_SELECTOR, "h3.ct-headline")
                job_title = job_title_element.text.strip() or job_title_element.get_attribute('innerHTML').strip()
            except NoSuchElementException:
                job_title = "No title found"

            try:
                job_url_element = job.find_element(By.CSS_SELECTOR, "a.ct-link")
