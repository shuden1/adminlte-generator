...
def scrape_jobs(file_path):
    ...
    try:
        job_elements = driver.find_elements(By.CSS_SELECTOR, "article.vacancy")
        for job_element in job_elements:
            try:
                job_title_element = job_element.find_element(By.CSS_SELECTOR, "h2")
                job_title = job_title_element.text.strip() or job_title_element.get_attribute('innerHTML').strip()
            except NoSuchElementException:
                job_title = "No Title"

            try:
                job_url_element = job_element.find_element(By.CSS_SELECTOR, "a[href]")
